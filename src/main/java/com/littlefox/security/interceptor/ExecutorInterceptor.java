                    ...
                    if (methodList.size() > 0 || methedName.indexOf(CrypticConstant._COUNT) > -1) {
                        ...
                        for (int i = 0; i < methodList.size(); i++) {
                            ...
                            for (Parameter p : params) {
                                boolean b1 = !AlgorithmExecutor.isJavaClass(p.getType());

                                boolean model = p.getType().getTypeName().indexOf("model") > -1;

                                boolean model1 = p.getName().equals("model");

                                boolean b2 = p.getType() == List.class;

                                boolean b3 = p.getDeclaredAnnotation(CrypticField.class) == null;

                                boolean b = !AlgorithmExecutor.isJavaClass(p.getType()) || p.getType().getTypeName().indexOf("model") > -1 || p.getName().equals("model") || p.getType() == List.class && p.getDeclaredAnnotation(CrypticField.class) == null;
